...
describe('component generator', () => {
  ...
  it('should run successfully', async () => {
    ...
    expect(
      tree.exists(
        joinPathFragments(sourceRoot, 'lib/ui-button/ui-button.spec.tsx')
      )
    ).toBe(true);

    expect(
      tree.exists(
        joinPathFragments(sourceRoot, 'lib/ui-button/ui-button.stories.tsx')
      )
    ).toBe(true);
    ...
    expect(
      tree.exists(
        joinPathFragments(
          e2eSourceRoot,
          'integration/ui-button/ui-button.spec.ts'
        )
      )
    ).toBe(true);
  });
